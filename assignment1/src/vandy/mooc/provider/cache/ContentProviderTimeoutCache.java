...
public class ContentProviderTimeoutCache
    ...
    private int putValues(String acronym,
        ...
        for (int i = 0; i < longForms.size(); i++) {
            ...
            cvs.put(AcronymEntry.COLUMN_LONG_FORM, longForms.get(i).getLf());
            cvs.put(AcronymEntry.COLUMN_FREQUENCY, longForms.get(i).getFreq());
            cvs.put(AcronymEntry.COLUMN_SINCE, longForms.get(i).getSince());
    ...
    public void removeExpiredAcronyms() {
        ...
        try (Cursor expiredData = mContext.getContentResolver().query(AcronymEntry.CONTENT_URI
            ...
            if (expiredData != null && expiredData.moveToFirst()) {
                do {
                    remove(expiredData.getString(expiredData.getColumnIndex(AcronymEntry.COLUMN_ACRONYM)));
